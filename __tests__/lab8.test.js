...
describe('Basic user flow for SPA ', () => {
  ...
  it('Test4: On first Entry page - checking page header title', async () => {
    ...
    const headerText = await page.evaluate(() => {
      ...
      expect(headerText).toEqual('Entry 1');
  });
  ...
  it('Test6: On first Entry page - checking <body> element classes', async () => {
    ...
    expect(testBody).toEqual('single-entry');
  ...
  it('Test7: Clicking the settings icon, new URL should contain #settings', async () => {
    ...
    expect(page.url().includes('settings')).toBe(true); //'http://127.0.0.1:5500/#settings'
  });
  ...
  it('Test9: On Settings page - checking <body> element classes', async () => {
    ...
    const bodyTest = await page.$eval("body", (page) => {
      return page.className;
    })
    expect(bodyTest).toBe('settings');
  });
  ...
  it('Test12: When the user is on the homepage, the header title should be “Journal Entries"', async () => {
    ...
    await expect(headerText).toEqual('Journal Entries');
  });
  
  // define and implement test13: On the home page the <body> element should not have any class attribute 
  it('Test13: On the home page the <body> element should not have any class attribute', async () => {
    const bodyElement = await page.$eval('body', (page) => {
      return page.classList.length;
    })
    expect(bodyElement).toBe(0);
    const emptyCheck = await page.evaluate(() => {
      return document.body.className;
    });
    expect(emptyCheck).toBe('');
  });
  ...
  it('Test15: On second Entry page - checking page header title', async () => {
    ...
    expect(headerText).toEqual('Entry 2');
  });
